...
const sms = (conn, m) => {
    ...
    // Message sending functions with group support
    const sendToGroup = (groupId, content, options = {}) => {
        if (!groupId.endsWith('@g.us')) {
            throw new Error('Invalid group ID. Group IDs must end with @g.us')
        }
        return conn.sendMessage(groupId, content, options)
    }
    
    m.reply = (text, id = m.chat, option = { mentions: [m.sender] }) => {
        const content = { text: text, contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyS = (stik, id = m.chat, option = { mentions: [m.sender] }) => {
        const content = { sticker: stik, contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyImg = (img, text, id = m.chat, option = { mentions: [m.sender] }) => {
        const content = { image: img, caption: text, contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyVid = (vid, text, id = m.chat, option = { mentions: [m.sender], gif: false }) => {
        const content = { video: vid, caption: text, gifPlayback: option.gif, contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyAud = (aud, id = m.chat, option = { mentions: [m.sender], ptt: false }) => {
        const content = { audio: aud, ptt: option.ptt, mimetype: 'audio/mpeg', contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyDoc = (doc, id = m.chat, option = { mentions: [m.sender], filename: 'undefined.pdf', mimetype: 'application/pdf' }) => {
        const content = { document: doc, mimetype: option.mimetype, fileName: option.filename, contextInfo: { mentionedJid: option.mentions } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.replyContact = (name, info, number, id = m.chat) => {
        const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + name + '\n' + 'ORG:' + info + ';\n' + 'TEL;type=CELL;type=VOICE;waid=' + number + ':+' + number + '\n' + 'END:VCARD'
        const content = { contacts: { displayName: name, contacts: [{ vcard }] } }
        return id.endsWith('@g.us') ? sendToGroup(id, content, { quoted: m }) : conn.sendMessage(id, content, { quoted: m })
    }
    
    m.react = (emoji) => conn.sendMessage(m.chat, { react: { text: emoji, key: m.key } })
    
    // Additional group-specific functions
    m.sendToGroup = (groupId, content, options = {}) => sendToGroup(groupId, content, options)
