...
async function connectToWA() {
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        ...
        //========== WORK TYPE ============ 
        const allowedModes = ['public', 'private', 'groups', 'inbox'];
        ...
        // Groups Mode: Only works in groups
            ...
            if (!isGroup) return reply("❌ This bot only works in groups!");
        }

        // Inbox Mode: Only works in private chats
        else if (config.MODE === 'inbox') {
            if (isGroup) return reply("❌ This bot only works in private chats!");
        }
